...
# Load Model (High-End Priority)
MODEL_NAME = "roberta-large-openai-detector" # Explicitly High End
try:
    print(f"Loading High-End Model: {MODEL_NAME}...")
    tokenizer = RobertaTokenizer.from_pretrained(MODEL_NAME)
    model = RobertaForSequenceClassification.from_pretrained(MODEL_NAME)
    MODEL_TYPE = "High-End Enterprise Model (OpenAI Detector)"
except Exception as e:
    print(f"Error loading {MODEL_NAME}: {e}")
    print("Falling back to local/default model...")
    # Fallback to local or base
    try:
        model_path = "models/roberta_detector_hq" if os.path.exists("models/roberta_detector_hq") else "models/roberta_detector"
        tokenizer = RobertaTokenizer.from_pretrained(model_path)
        model = RobertaForSequenceClassification.from_pretrained(model_path)
        MODEL_TYPE = "Local Custom Model"
    except:
        # Final fallback
        ...
        MODEL_TYPE = "Base OpenAI Model"

model.eval()
if torch.cuda.is_available():
    model.to('cuda')
    print("Model loaded on CUDA (Fast)")
else:
    print("Model loaded on CPU (Standard)")
...
def apply_heuristics(text, ai_score, human_score):
    ...
    # 6b. Self-Disclosure / Meta-Reference Detection (New)
    ...
    if any(p in lower_text for p in disclosure_phrases):
        ai_score = 100
        human_score = 0
